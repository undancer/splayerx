...
//     if (existsSync(path)) {
//       splayerxProxy.getMediaInfo(path, info => reply(event, 'media-info-reply',
//         null, info));
//     } else {
...
//     finished: boolean,
//   }>> = new Map();
//   ipcMain.on('subtitle-metadata-request',
//     async (
//       event: IpcMainEvent,
//       videoPath: string,
//       streamIndex: number,
...
//               if (data) {
//                 if (isImage) {
//                   subtitle.metadata = data.subarray(4, 4 + data.readUInt32LE(0))
//                     .toString('utf8');
//                 } else {
...
//               if (error === 'EOF') {
//                 try {
//                   await outputFile(subtitle.path,
//                     isImage ? subtitle.cache : subtitle.cache.toString('utf8'));
//                   delete subtitle.cache;
//                   subtitle.finished = true;
//                   reply(event, 'subtitle-metadata-reply',
//                     undefined, subtitle.finished, subtitle.path);
//                 } catch (err) {
//                   reply(event, 'subtitle-metadata-reply', err);
//                 }
//               } else if (error || !data) {
//                 reply(event, 'subtitle-metadata-reply',
//                   new Error(error || 'Missing subtitle data.'));
//               }
//               reply(event, 'subtitle-metadata-reply',
//                 undefined, subtitle.finished, subtitle.metadata);
//             });
//         } else {
//           reply(event, 'subtitle-metadata-reply',
//             undefined, subtitle.finished, subtitle.metadata);
...
//       }
//     });
//   ipcMain.on('subtitle-cache-request',
//     async (event: IpcMainEvent, videoPath: string, streamIndex: number) => {
...
//             reply(event, 'subtitle-cache-reply',
//               undefined, subtitle.path);
//           } else {
...
//                       const metadataInfo = data.subarray(4, 4 + data.readUInt32LE(0))
//                         .toString('utf8');
//                       if (metadataInfo === subtitle.metadata) {
//                         data = data.slice(4 + metadataLength);
//                       }
//                     }
//                   } else {
...
//         } else {
//           reply(event, 'subtitle-cache-reply',
//             new Error('Missing subtitle entry, should request metadata first.'));
//         }
//       } else {
//         reply(event, 'subtitle-cache-reply',
//           new Error('Missing videoPath entry, should request metadata first.'));
//       }
//     });
//   ipcMain.on('subtitle-stream-request',
//     (event: IpcMainEvent, videoPath: string, streamIndex: number, time: number) => {
//       const streamSubtitlesMap = videoSubtitlesMap.get(videoPath);
//       if (streamSubtitlesMap) {
//         const subtitle = streamSubtitlesMap.get(streamIndex);
//         if (subtitle) {
//           splayerxProxy.extractSubtitles(
//             videoPath, streamIndex, time, true, 20,
//             (error, pos, isImage, data) => {
//               if (data) {
//                 reply(event, 'subtitle-stream-reply', undefined, data);
//               } else {
//                 reply(event, 'subtitle-stream-reply',
//                   new Error(!error || !data ? 'Missing subtitle data' : error));
//               }
//             },
//           );
//         } else {
